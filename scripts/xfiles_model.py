...
def create_scoring_engine(p, q, vocab, prior=0.1):
    ...
    def predict(phrase):
        ...
        for w in clean:
            vec[[index_of[w]]] += 1
        b = np.log(prior)
        w = np.sum(np.log(p) * vec)
        resp = b + w

        b = np.log(1 - prior)
        w = np.sum(np.log(q) * vec)
        not_resp = b + w
...
def fit_bayes(df):
    ...
    for i, w in enumerate(vocab):
        p[i] = (pos_counts[w] + alpha) / (pos_total + K * alpha)
        q[i] = (neg_counts[w] + alpha) / (neg_total + K * alpha)
